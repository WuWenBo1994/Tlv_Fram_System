![image-20251206142715522](Readme.assets/image-20251206142715522.png)基于TLV架构的铁电存储文件系统
...
```
typedef struct {    uint32_t magic;             // 魔数 0x544C5646    uint16_t version;           // 格式版本    uint16_t tag_count;         // 活动标签计数    uint32_t data_region_start; // 数据区起始地址    uint32_t data_region_size;  // 数据区大小    uint32_t next_free_addr;    // 下一个可分配地址    uint32_t total_writes;      // 总写入操作数    uint32_t last_update_time;  // 最后更新时间戳    uint32_t free_space;        // 可用空间    uint32_t used_space;        // 已用空间    uint32_t fragment_count;    // 碎片计数    uint32_t fragment_size;     // 碎片大小    uint8_t reserved[210];      // 保留扩展    uint16_t header_crc16;      // 头部 CRC16} tlv_system_header_t;
```
